...
function stableStringify(result: unknown): string {
  if (result === null || typeof result !== 'object') return JSON.stringify(result);
  if (Array.isArray(result)) return `[${result.map(stableStringify).join(',')}]`;
  const keys = Object.keys(result as Record<string, unknown>).sort();
  return `{${keys.map(k => `${JSON.stringify(k)}:${stableStringify((result as any)[ k ])}`).join(',')}}`;
}
...
function getPingPongCount(currentHash: string): number {
  ...
  for (let i = history.length - 1; i >= 0; i--) {
    ...
    count++; // expected 从 last 改为 other
  }
  // [A,B,A,B]: count = 4，currentHash = A，lastHash = B ,otherHash = A，返回 5
