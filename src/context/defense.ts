...
// 调 LLM 本身就要花时间和钱。用 LLM 压缩一次对话可能要几秒钟、几千 token 的额外消耗。所以 LLM 压缩只在即时防线不够用的时候才触发
// 在一个典型的 50 轮编程对话中，工具结果占上下文的 60-80%。三层即时防线主要就是在对这 60-80% 动手——截断超大的、清理过期的、估算剩余的。
// 对话历史本身（user/assistant 消息）通常只占 20-30%，那是 LLM 摘要压缩要处理的部分。
...
export function applyDefense(
  ...
  console.log(`[Layer 2: 截断] ${trunc.truncated} 个超长结果被截断`);

  // Layer 3: TTL prune old tool results
  const prune = ttlPrune(result, timestamps);
  result = prune.messages;

  console.log(`[Layer 3: TTL] ${prune.softPruned} 个软修剪, ${prune.hardPruned} 个硬清除`);
